...
// bm13-spec §5/§Verification checklist, revised bm20-spec §Implementation
// §4 — billmgmt-code-standards.md §5.1 now locks the surface to exactly
// THREE handlers ("a sanctioned third M2M handler (architecture decision)" —
// bm20 IS that decision, recorded in code-standards §5): the two bm04
// handlers plus `.../distribution/outcome`. `tests/app/route-manifest.test.ts`
// enumerates `app/**/page.tsx` only (a Route Handler is not a page), so this
// is the M2M surface's own inventory lock — the two-page test's counterpart
// for the module's other mutation surface (code-standards §8's permission-map
// table row count).
...
const EXPECTED_HANDLERS = [
  ...
  "[runId]/distribution/outcome/route.ts",
] as const;
...
describe("app/api/billrun/** route inventory (bm13 ship gate, revised bm20)", () => {
  it("has exactly the three documented M2M Route Handlers, no more, no fewer", () => {
