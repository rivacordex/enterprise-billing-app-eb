  ...
  await sql.begin(async (tx) => {
    ...
    await tx`
      CREATE TEMP TABLE _bm29_resolved ON COMMIT DROP AS
        WITH asof AS (
          SELECT offering_id, unit_price, price_ref, envelope_price_type,
          ...
          FROM (
            SELECT pop.product_offering_id                       AS offering_id,
                   ...
                   pop.price_component ->> 'priceType'           AS envelope_price_type,
        ...
        SELECT pi.product_offering_id                                   AS product_offering_id,
               ...
               a.envelope_price_type                                   AS envelope_price_type,
               (oipo.order_item_price_override_id IS NOT NULL)         AS has_override,
    ...
    await tx.unsafe(`
      ...
      BEGIN
        SELECT count(*) FILTER (WHERE unit_price IS NULL),
               count(*) FILTER (WHERE unit_price IS NOT NULL AND NOT has_override
                                        AND envelope_price_type IS DISTINCT FROM 'recurring'),
               count(*) FILTER (WHERE unit_price IS NOT NULL AND currency IS DISTINCT FROM account_currency)
          INTO v_not_found, v_unsupported, v_currency_mismatch
        FROM   _bm29_resolved;
        ...
        IF v_unsupported > 0 THEN
          RAISE EXCEPTION 'RECURRING_PRICE_UNSUPPORTED (HARD): % subscription(s) resolve to the offering''s current flat fee being a one-time charge, with no override (D33/Inv #28)', v_unsupported;
        END IF;
