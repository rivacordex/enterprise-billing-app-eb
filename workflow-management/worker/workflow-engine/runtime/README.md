...
| `rl.py` | **rm09's real Rating Loader** (`python3 -m runtime.rl`): consumes RP's rated manifest and, in ONE psycopg transaction (Inv #8), runs the `BILL_APPROVED` guard (refuse whole batch → `LOAD_BLOCKED_BILLED`/`REFUSED`), the `CURRENCY_MISMATCH` assertion, **rm10's** batch-level supersession by `file_key` (status-only, all partitions, lineage on the retired `udr_batch` rows, `CROSS_PERIOD_SUPERSEDE`/`SHRINKING_REISSUE`), the bulk `COPY` insert at `RATED`, and reconciliation (`parsed = rated + rejected + discarded`; imbalance → `RECON_IMBALANCE`/`FAILED`); then — **only after commit** — the idempotent cross-boundary `landing/` → `archive/` archive (copy-then-delete, `archive_file_path` set last) | rm09 owns the transaction boundary + archive; rm10 owns the supersession it fills in |
...
| `stranded_reconcile.py` | **rm11's stranded-batch recovery** (`python3 -m runtime.stranded_reconcile`), invoked by `../../flows/rating-batch-reconcile.yaml`: finds `udr_batch` rows stuck at `PROCESSING` beyond a namespace-KV threshold, resolves each (`FAILED`, releasing the `UNIQUE (file_key, batch_run_num)` claim so the file reprocesses), and emits one `BATCH_STRANDED` (`MAJOR`) line per resolved batch | rm11 owns this outright — a new flow, not a filled-in stub |
| `completeness_check.py` | **rm12's completeness and gap detection** (`python3 -m runtime.completeness_check`), invoked by `../../flows/rating-completeness-check.yaml`: compares `udr_batch` against a per-`udr_type` expected-cadence KV config, raises `FILE_NOT_RECEIVED`/`FILE_LATE` (idempotent per `alarm_key`), clears them itself once a later batch for that period lands and completes, and surfaces superseded-never-replaced usage via the rm01 orphan index | rm12 owns this outright — a new flow, not a filled-in stub |
