export const AUDIT_EVENT_TYPES = [
  "LOCAL_LOGIN",
  "SSO_LOGIN",
  "USER_FIRST_LOGIN",
  "USER_CREATED",
  "USER_UPDATED",
  "USER_DISABLED",
  "USER_ENABLED",
  "USER_DELETED",
  "USER_LOCKED",
  "USER_UNLOCKED",
  "USER_PASSWORD_RESET",
  "USER_PASSWORD_CHANGED",
  "USER_AUTH_METHOD_CHANGED",
  "ROLE_CREATED",
  "ROLE_UPDATED",
  "ROLE_DELETED",
  "ROLE_ASSIGNED",
  "ROLE_REVOKED",
  "PERMISSION_MAPPING_CHANGED",
  "SYSTEM_CONFIG_CHANGED",
  "ORGANIZATION_CREATED",
  "CUSTOMER_CREATED",
  "ORGANIZATION_UPDATED",
  "ORGANIZATION_STATUS_CHANGED",
  "CUSTOMER_STATUS_CHANGED",
  "PARTY_ROLE_SPECIFICATION_UPDATED",
  "CONTACT_CREATED",
  "CONTACT_UPDATED",
  "CONTACT_DELETED",
  "PREFERRED_CONTACT_CHANGED",
  "PREFERRED_METHOD_CHANGED",
  "PRODUCT_OFFERING_CREATED",
  "PRODUCT_OFFERING_UPDATED",
  "PRODUCT_OFFERING_BRANCHED",
  "PRODUCT_SPECIFICATION_CREATED",
  "PRODUCT_SPECIFICATION_UPDATED",
  "PRODUCT_SPECIFICATION_DELETED",
  "PRODUCT_PRICE_ADDED",
  "PRODUCT_PRICE_UPDATED",
  "PRODUCT_PRICE_DELETED",
  "PRODUCT_OFFERING_SUBMITTED_FOR_TESTING",
  "PRODUCT_OFFERING_RETURNED_TO_DRAFT",
  "PRODUCT_OFFERING_ACTIVATED",
  "PRODUCT_OFFERING_SUPERSEDED",
  "PRODUCT_OFFERING_OBSOLETED",
  "PRODUCT_OFFERING_RETIRED",
  "PRODUCT_OFFERING_DELETED",
  "PRODUCT_ORDER_CREATED",
  "PRODUCT_ORDER_PENDING_APPROVAL",
  "PRODUCT_ORDER_APPROVED",
  "PRODUCT_ORDER_REJECTED",
  "PRODUCT_ORDER_COMPLETED",
  "PRODUCT_ORDER_FAILED",
  "PRODUCT_INVENTORY_CREATED",
  "PRODUCT_INVENTORY_CHARACTERISTICS_UPDATED",
  "PRODUCT_INVENTORY_SUSPENDED",
  "PRODUCT_INVENTORY_RESUMED",
  "PRODUCT_INVENTORY_TERMINATED",
  "ACCOUNTS_ONBOARDED",
  "DOCUMENT_POSTED",
  "PERIOD_CLOSED",
  "JOURNAL_EXPORTED",
  "REASON_CODE_CHANGED",
  "BILL_CYCLE_CHANGED",
  "WIZARD_DEFAULTS_CHANGED",
  "ACCOUNT_CLOSED",
  "BILL_RUN_MATERIALIZED",
  "BILL_RUN_TRIGGERED",
  "BILL_RUN_RERUN",
  "BILL_RUN_APPROVED",
  "BILL_RUN_REJECTED",
  "BILL_RUN_POSTED",
  "BILL_RUN_CANCELLED",
  "BILL_RUN_RECONCILED",
  "BILL_RUN_DISTRIBUTION_STARTED",
  "BILL_RUN_DISTRIBUTION_RERUN",
  "BILL_RUN_DISTRIBUTION_ABANDONED",
] as const;

export type AuditEventType = (typeof AUDIT_EVENT_TYPES)[number];

export type { AuditLog, AuditLogInsert } from "@/db/schema/audit";
