...
18. **Human-readable IDs** (architecture §3): domain-table IDs are a fixed prefix + an **8-digit** zero-padded per-table DB sequence (e.g. `PRDOFR00000001`). ID assembly happens in the DB layer (default expression or repository insert) — never in services or UI. Validators use `^PREFIX\d+$` (shape, not a fixed digit count) for the sequence-widened IDs, so legacy-narrower rows and future width changes both stay valid; the always-8-digit IDs (`party_role`, `contact_medium`, `document_line`, `document`) keep `^PREFIX\d{8}$`.
