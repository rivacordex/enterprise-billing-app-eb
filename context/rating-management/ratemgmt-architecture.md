...
| --- | --- | --- |
| **Rating compute + orchestration** | **Kestra (OSS edition)** — a namespace on the shared workflow-management platform | Both the orchestrator **and the compute engine**. Rating logic is expressed in the workflow definitions themselves, not only in called code — which is why `rating_flow_revision` is stored on every rated row (§6, Inv #12). Vendor-neutral naming is **not** used in this module: unlike Bill Run, the engine here is load-bearing and named. **Platform reconciliation:** per `context/workflow-management/wfm-architecture.md`, Kestra is the solution's one shared **workflow-management platform** and rating is its **function 1**; the physical engine is named **`workflow-engine`**, and a "separate/dedicated deployment" is the platform's **split-by-module topology** — one valid mapping, with the default being a single shared instance — not a standalone rating decision. |
...
**This module spans two surfaces.** That is the primary structural delta: `architecture.md` §2 describes folder ownership inside one deployable, and the rating module's logic does not live there.

> **Platform reconciliation (`wfm-architecture.md` §4).** The rating logic surface is no longer a separate repo but the **`workflow-management/` spin-off subdirectory** of the app repo (a monorepo today, structured to spin off later). The `*(rating repo)*` paths below now resolve as: `flows/**` → `workflow-management/flows/rating-engine/**`; `worker/**` → `workflow-management/worker/workflow-engine/**` (the one shared engine image); `dev/**` → `workflow-management/dev/**`. The Azure bicep stays at the app-root `infra/`, renamed to the `workflow-engine` identity — it is **not** under `workflow-management/`.
...
6. **[CRITICAL] A batch colliding with a live `BILL_APPROVED` row is refused whole.** RL writes nothing and raises `LOAD_BLOCKED_BILLED`. This is a deliberate exception to the record-level default: a collision with an approved invoice means the file's assumptions about the period are wrong, which is a human decision, not a per-record disposition.
   ...
   **In-flight amendment (bm25, authorized — Khek, module owner, 2026-09-13).** A batch colliding with a live **`BILL_DRAFT`** row — records a bill run holds *in flight*, before approval — is **also refused whole**, raising `LOAD_BLOCKED_INFLIGHT` at **`MINOR`** (recoverable: reject/rerun the run or wait for it to finish, then reload; unlike the `BILL_APPROVED` collision's financial finality). A billed collision takes precedence when both are present. The refusal became necessary once bm24 made reject/rerun/cancel *release* claimed rows back to `RATED`: a fresh load landing mid-run would otherwise collide with the `BILL_DRAFT` rows the run still owns. **The guarantee is the `rating.rating_status_guard` trigger, not the pre-check.** RL's widened pre-check (`status IN ('BILL_DRAFT','BILL_APPROVED')`) is only a readable whole-batch refusal — it loses the check-then-claim TOCTOU race; the `BEFORE UPDATE` trigger on `rating.udr_rated` refuses any `rating_runtime` UPDATE that would supersede a row held at `BILL_DRAFT`/`BILL_APPROVED`, so the invariant survives a bypassed pre-check. The `BILL_APPROVED` clause above is untouched.
