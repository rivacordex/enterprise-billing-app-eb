...
**What at-least-once means for consumers.** `process_log` is operational telemetry consumed by external alerting/monitoring, not a source of financial truth (rating amounts live in `udr_rated`, which this sweep never writes), so a rare duplicate is a monitoring-hygiene issue, not a correctness one. A duplicated row carries the *same* content — identical `event_code` and correlation fields — with only a fresh `log_id`/`insert_datetime`. Alert rules already key off `event_code` presence (Inv #14, never log text), so a one-off duplicate never changes *whether* an alarm fires; any **count-based** consumer must dedupe on the log-line content (every column except the regenerated `log_id`/`insert_datetime`) or tolerate a rare over-count until the durable-marker fix lands.
