...
> **bm25 amendment (widened guard + trigger, authorized — Khek, module owner, 2026-09-13).** The pre-check filter is widened to `status IN ('BILL_DRAFT','BILL_APPROVED')` and the helper renamed `find_billed_or_inflight_collisions`. A `BILL_APPROVED` collision still raises `LOAD_BLOCKED_BILLED` (`MAJOR`); a `BILL_DRAFT` collision — records a bill run holds *in flight* — raises the new `LOAD_BLOCKED_INFLIGHT` (`MINOR`, recoverable) naming the blocking `bill_run_id`, with billed taking precedence when both are present. Crucially, the widened pre-check is **only a readable error** — it loses the check-then-claim TOCTOU race — so the guarantee is a database trigger, `rating.rating_status_guard` (in `db/bootstrap/rating-db-roles.sql`, bm25 §2): a `BEFORE UPDATE` on `rating.udr_rated` that refuses any `rating_runtime` UPDATE superseding a row held at `BILL_DRAFT`/`BILL_APPROVED`. See `context/billing-management/specs/bm25-rating-inflight-guard.md` and `ratemgmt-architecture.md` Inv #6.
