...
- rm01 — `rating` schema foundation (`specs/rm01-rating-schema-foundation.md`). Delivered:
  ...
  - `package.json` — added `db:setup-partman-rating`, wired into `db:setup`.
...
- `rating.event_catalog.default_severity` is documented as "CHECK" in rm01-spec §Implementation §5's column notes, but no CHECK SQL is given (unlike every other CHECK in the spec, which is spelled out). Implemented with no CHECK constraint, matching the spec's Implementation section literally — flag if rm02 (the seed unit) expects one already in place.
