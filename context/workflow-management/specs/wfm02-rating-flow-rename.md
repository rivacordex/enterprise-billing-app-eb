...
- [ ] `workflow-management/flows/rating-engine/` holds exactly the four **new** filenames; each file's `id:` matches its filename; the ONLY edits inside each flow body are the §1 `id:` line and the §2 in-flow comment cross-reference fixes — every other byte is identical to pre-rename (`git diff -M` of each flow shows only those documented lines, no task/trigger/logic change).
- [ ] `grep -rn "ran-usage-rating\|log-sweep\|stranded-batch-reconcile\|completeness-check" enterprise-billing-app | grep -v "rating-completeness-check"` returns **only** intended survivors: the `log-sweep:`/`LOG_SWEEP` log-string prefixes, the flow-local task id `- id: completeness-check`, and the `stranded` KV key (D-B), and historical `rmNN` spec bodies (§8) — **no** live `id:`, filename, test, docstring, or README reference to an old name. (The `-v "rating-completeness-check"` excludes the **new** id/filename, which the bare `completeness-check` pattern would otherwise substring-match and falsely report as stale; the other three old names are not substrings of their new counterparts.)
