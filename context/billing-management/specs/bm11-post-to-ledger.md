...
- **Per-account posting transaction** `services/billing/post-run.ts` → `postAccount(run, banId)` in its **own `db.transaction`** (never one giant transaction — releases the shared `sys.revenue`/`sys.tax_payable` locks between accounts, makes a partial run resumable):
  ...
  1b. **Skip the zero-value invoice (ADDED 2026-09-17, owner decision)** — the bill's `subtotal` **and** `total_amount` are both exactly zero → mark the account `SKIPPED` / `ZERO_TOTAL_NOT_INVOICED` and return `skipped`, consuming **no** invoice number (Inv #7). A bill worth nothing has no economic substance (no receivable, no revenue, no tax), and posting it FAILS anyway: step 3 inserts the revenue `charge` line at `amount = subtotal`, which `document_line_amount_check` (`amount > 0`) rejects, parking the account. The account must leave `PROCESSED` (not stay parked) or run completion below can never fire; it remains visible on the run and on the Uncharged tab (Inv #22), and `SKIPPED` is **not** a billed marker, so the account is fully billable next period. **Deliberately narrow — both guarded by `[CRITICAL]` tests:** a **negative** total is a credit position with real economic substance and is NEVER skipped (it keeps failing loudly until a credit-note path exists — out of scope, plan §13); and `subtotal = 0` with a non-zero `total_amount` (tax-only) is NEVER skipped, since that would drop a real tax liability. Compared via `money.compare` (exact integer sen), never `Number()` on an amount (code-standards §2.2). **Revisit when discounting ships:** a bill can then net to zero because a charge was fully discounted. That is NOT a revenue-recognition exposure — net IS the transaction price, this step books the GL revenue line at `amount = subtotal` (net), and the gross/discount split persists on `customer_bill_line` whether or not the bill posts. The exposure is operational: the predicate cannot distinguish a fully-discounted account from a no-charge one, so such an account would wedge the run at approval (`positive_totals`) and, once unblocked, receive no invoice despite having had real billable activity. See `billmgmt-known-issues.md` §12.
