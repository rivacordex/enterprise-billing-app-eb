...
- **Orphans are surfaced, resolvable or not (D32).** The exception read `LEFT JOIN`s `udr_subscriber_ref_id → product_inventory → billing_account`: a resolvable orphan shows its account name; an **unresolvable** one (no `product_inventory`) shows the subscriber ref with a NULL account — never dropped. Scoped to the run's `period_partition` and the unclaimed leftovers Collection left (`status = 'RATED'`, `billrun_ban_id IS NULL`, `is_live`) plus `BILL_NOTUSED` rows.
...
- `BILL_NOTUSED` rows: `rating.udr_rated` where `status = 'BILL_NOTUSED'`, `is_live`, `partition_period` = the run period.
- **Orphans:** `rating.udr_rated` where `status = 'RATED'`, `billrun_ban_id IS NULL`, `is_live`, `udr_type = 'RAN_USAGE'`, `partition_period` = the run period — `LEFT JOIN product_inventory → billing_account` for the account name (NULL = unresolvable subscriber, shown by `udr_subscriber_ref_id`).
