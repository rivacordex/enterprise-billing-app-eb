...
- [ ] A per-account HARD stage failure POSTs `FAILED`/`HARD` for that `(ban, stage)`; that account reaches `PROCESSING_FAILED` while every other account still bills; once all accounts are terminal the run recomputes to `PROCESSED` (the established `compute-run-status.ts` contract — the failed account is `SKIPPED` at approval) and stays rerunnable.
