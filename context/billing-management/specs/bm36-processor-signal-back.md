...
- **Run-level terminal settlement (Inv #1).** `errors: on_error` POSTs `/api/billrun/{run}/status` `{status: "PROCESSING_FAILED"}` (with `retry`, **no** `allowFailure` — this one must land) so a flow that errored before every account signalled settles the run instead of hanging. `finally: on_finally` POSTs the same **only when the execution did not succeed** (`{{ execution.state.current != 'SUCCESS' }}`) — this covers a KILL, which fires `finally` but not `errors`. On a healthy run `on_finally` is a no-op, because the per-account `verification` `DONE` signals already drove the run to `PROCESSED`. `handle-status-push.ts` accepts `PROCESSING_FAILED` only for a `PROCESSING` run and is idempotent on a late/duplicate push.
...
The account's stage group carries an `errors:` handler that captures the failing stage/detail and POSTs the FAILED signal, then lets the iteration complete (error handled) so sibling accounts continue:
...
errors:
  - id: signal_account_failed
    type: io.kestra.plugin.core.http.Request
    retry: { type: constant, interval: PT5S, maxAttempt: 2 }
    uri: "http://app:3000/api/billrun/{{ inputs.bill_run_id }}/stage/{{ <failing-stage> }}/complete"
    method: POST
    contentType: application/json
    headers: { Authorization: "Bearer {{ secret('BILLRUN_APP_TOKEN') }}" }
    body: |
...
The failing stage id and the psql `RAISE EXCEPTION` message are read from the errored task's context (`error_code` falls back to a generic `PROCESSING_STAGE_FAILED` when the stage did not emit a structured code — e.g. bm27 `CURRENCY_MISMATCH`, bm29 `RECURRING_PRICE_UNSUPPORTED` do).
...
errors:
  ...
  - id: on_finally
    type: io.kestra.plugin.core.http.Request
    runIf: "{{ execution.state.current != 'SUCCESS' }}"
    retry: { type: constant, interval: PT5S, maxAttempt: 2 }
    uri: "http://app:3000/api/billrun/{{ inputs.bill_run_id }}/status"
    method: POST
    contentType: application/json
...
- **`billmgmt-architecture.md`** — flip the processor signal-back description from **stub → real**: the processing flow now POSTs per-stage `DONE` and terminal `PROCESSING_FAILED` via `http.Request`, at parity with the distributor (bm34). Add a one-line decision / Resolved-Ambiguity note on the **processing↔distribution terminal asymmetry**: processing self-completes via the per-account `verification DONE` signal (there is **no** run-level `PROCESSING_FINISHED` recompute push, unlike distribution's `DISTRIBUTION_FINISHED`), so `on_finally` fires a terminal `PROCESSING_FAILED` only on non-`SUCCESS`. **No new Module Invariant** — this fulfils the existing Inv #1 (always report a terminal status) and D6 (write-then-signal); do not inflate the invariant count.
...
- [ ] `on_error` and `on_finally` POST `/status {status: "PROCESSING_FAILED"}` (no `Log` stub remains); an errored or killed run settles rather than hanging; a healthy run's `on_finally` is a no-op (`runIf` state guard).
