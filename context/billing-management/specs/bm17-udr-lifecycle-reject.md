...
> **bm25 amendment (2026-09-15) — the re-claim source set narrowed to `RATED` only.** The `status IN ('RATED','REJECTED') → BILL_DRAFT` clause above is superseded: bm24 made reject/rerun/cancel **release** claimed rows back to `RATED` (four claim columns NULLed) instead of parking them at `REJECTED`, so no billing path produces a `REJECTED` `udr_rated` row any longer, and bm25 narrowed `billrun_status_guard` (`billrun-db-roles.sql` Step 7b) to permit only `RATED → BILL_DRAFT`. The real Collection stage (bm27) must claim `status = 'RATED' → BILL_DRAFT`; a `REJECTED → BILL_DRAFT` claim is now **refused at the DB**. The rest of T6 (attempt re-stamp, sole-re-claimer, marker lifecycle) is unchanged. See `context/billing-management/specs/bm25-rating-inflight-guard.md` §Implementation §3.
