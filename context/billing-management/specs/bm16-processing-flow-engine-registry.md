...
- **Flow surface layout (reconciled — `wfm-architecture.md` §4 / `wfm01`).** Bill-run template skeletons live under **`workflow-management/flows/bill-run-processor/`** (this spec) and **`workflow-management/flows/bill-run-distributor/`** (bm20) — function-first, not `flows/billrun/`. `workflow-management/flows/rating-engine/` is the co-located function-1 surface (rating's real flows), not a "reserved sibling in a separate repo": the whole `workflow-management/` tree is one monorepo surface structured to spin off later. Skeletons carry key sections + commented `# STUB:` activities, **no business logic**; they deploy as flow shells on stand-up (`wfm01` §7b). Recorded in `billmgmt-architecture.md` §2 and `billmgmt-code-standards.md` §7.
- **The engine is addressed by name (D24).** `engine-registry.ts` wraps the as-built `engine-client.ts` (`real`/`stub`, `isBillRunEngineConfigured`, Basic-Auth from Key Vault) and resolves a logical engine — `billrun` — to a physical connection (base URL, namespace, credentials). Topology (one shared instance vs two) is a deploy-time mapping, changeable without app-code change (D25) — the authoritative topology model is `wfm-architecture.md` §5, driven by the `topology` deploy parameter (default `collapsed`: one `workflow-engine` instance hosting both namespaces). The **resolved engine identity** is stamped on the run per execution (D25e), so a later topology change never orphans reconcile/cancel of a historical execution.
