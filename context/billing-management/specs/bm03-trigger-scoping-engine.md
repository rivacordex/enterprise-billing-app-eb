...
- **Idempotent re-trigger (real-engine contract, not v1 behavior):** the engine call is **not** transactional, so `startExecution` can succeed on the wire and then the surrounding transaction can still roll back (a later write fails, or the connection drops before commit) — leaving an execution running on the engine that the app has no record of. A retrigger would then start a **second** execution. The stable idempotency key is **`(bill_run_id, attempt)`** — already carried in the payload, uniform across retries of the same attempt. The **real** engine client MUST send it as the engine's dedup/idempotency key, and the engine MUST return the **existing** execution for a repeated key instead of starting a new one, so a succeed-then-rollback retry reuses the same execution. This is a contract for the real integration only: v1 runs the **stub** client (no HTTP; `stub-exec-{bill_run_id}` is deterministic, hence already idempotent) and the real engine is undeployed, so no duplicate-execution path exists in v1. A future unit that wires the real engine lands the header/dedup wiring **and** the succeed-then-rollback-then-retry test alongside it.
