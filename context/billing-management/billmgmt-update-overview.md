...
2. Replace the `on_error`/`on_finally` `Log` stubs with a real terminal `/api/billrun/{runId}/status` POST, so a HARD-failing or partial run settles deterministically to `PROCESSING_FAILED` instead of relying on the stall timeout.
...
9. **Failure path.** A HARD-failing account's stage POSTs `FAILED` and the terminal `/status` settles it to `PROCESSING_FAILED` (not the stall timeout); the run reflects it and is rerunnable. A forced mandatory distribution failure yields `DISTRIBUTION_FAILED`, rerunnable for only the failed artifacts.
...
2. A HARD-failing account reaches `PROCESSING_FAILED` via the terminal `FAILED`/`/status` POST (not the stall timeout), and the run reflects it and is rerunnable.
