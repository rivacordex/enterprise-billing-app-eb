...
| `actions/**` (billing slice) | Server Actions for **materialize, trigger, rerun, cancel, approve, post** — each wrapping validate → mutate → audit in one transaction. | DB queries; business logic beyond orchestration. |
...
| **Cache** | **None** | Per platform. Run status and totals are **derived live** (from `bill_run_account`) and **never cached**; any stored counter must equal the derived value (asserted by test). |
