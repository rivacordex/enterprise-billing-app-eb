...
| `workflow-management/flows/bill-run-processor/**`, `.../bill-run-distributor/**` | **Function-first flow surface** (`wfm-architecture.md` §4 — was `flows/billrun/`). Carries **template skeletons** for the bill-run processing/distribution pipelines: key sections + commented key activities documenting the app-side stage contract, **no business logic**. Deployed as flow *shells* on stand-up (`wfm01` §7b); the real logic is authored/versioned to this contract. Co-located sibling `workflow-management/flows/rating-engine/**` holds rating's real flows — one monorepo surface, structured to spin off later, not a separate repo. | Business logic; anything computing money inside the flow this phase. |
