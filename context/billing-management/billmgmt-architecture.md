...
| `flows/billrun/**` | **bm16 deliberate deviation.** Unlike `flows/rating/` (a reserved, untouched sibling — rating keeps ALL its flow YAML in a separate repo, none in this one), this app repo carries **template skeletons** for the bill-run processing pipeline: key sections + commented key activities documenting the app-side stage contract, **no business logic**. Not deployed from this repo — the real `bill_run_processing` flow is authored, versioned, and released from a separate workflow-management repository, built to this contract (`flows/billrun/README.md`). | Business logic; anything actually deployed to Kestra. |
