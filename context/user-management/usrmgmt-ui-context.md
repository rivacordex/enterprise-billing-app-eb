...
## UI Context: Color, Typography & Shape Tokens

**Aesthetic:** Formal telecoms / national carrier feel, inspired by Digital Nasional Berhad. Deep, corporate **indigo-navy** base; a signature **5G magenta → violet** energy accent; **cyan "connectivity"** secondary; clean, slightly-cool neutrals. Light-mode-first for data-dense admin screens (tables, forms), with a dark navy chrome (nav/sidebar) echoing the telecom hero look.

All tokens are given as CSS custom properties (usage notes below the tables).

---

## 0. Module Scope — What's Specific to User Management

This module reuses the brand system below **unchanged**: the brand scales, neutrals, typography, radius, and elevation are shared core, inherited by every later module (Product, Customer, Billing Service, Bill Run, Accounting). What is **specific to User Management** is the semantic wiring of those tokens to this module's domain objects, plus two deliberate exclusions:

1. **The AI / Iris-violet family and `--gradient-ai` are NOT used in User Management v1.** Architecture §7 states the module has **no AI/ML components**, so the AI tokens (§4) are _reserved for later modules_ (e.g. billing-anomaly flags) and must not appear on any v1 admin screen. Defining them is fine; using them here is a scope violation.
...
New module-specific semantic tokens added below wire each to a named component from code-standards §4.8 / §9: **user-status** (the exact `PENDING | ACTIVE | DISABLED | DELETED` set + locked overlay, §3.4 → `StatusBadge`), **auth-method** (`SSO | LOCAL`, §3.5 → `AuthMethodBadge`), **RBAC role + permission-level** (§3.6 → `RoleBadge`, `PermissionLevelTag`), and **audit-event category** colors (§3.7 → `AuditLogTable`). Per code-standards §4.3, define these as CSS variables in `globals.css`; never hardcode the hex in a component.
...
| `--action-cta-bg`            | `#E6007E` (accent-500)  | Featured CTA (e.g. "Invite user") |
...
**User-status mapping (authoritative for `UserStatus`).** `StatusBadge` (code-standards §4.8) maps the four `UserStatus` values to fixed tokens. Render each as a pill (`--radius-pill`) pairing the `-bg` tint with its `-fg` text, plus an icon so the two red states stay distinguishable without relying on color alone:
...
### 3.5 Auth-Method Badges (`AuthMethodBadge`)
...
### 3.6 RBAC — Role & Permission-Level Badges
...
### 3.7 Audit-Log Event Categories (`AuditLogTable`)
...
## 4. AI & Accent Variants — Reserved (not used in User Management v1)

> **Not used in this module.** Architecture §7: User Management has **no AI/ML components**. The Iris/violet family and `--gradient-ai` below are defined for _later_ modules (billing-anomaly flags, suggested roles, natural-language search) and must not appear on any v1 admin screen (see §0).

A distinct **Iris/violet** family separates AI-assisted features (smart user grouping, anomaly/billing-risk flags, suggested roles, natural-language search) from standard brand actions — so "AI did this" is always visually legible.
...
> Reserve `--gradient-5g` / `--gradient-brand` for marketing-style moments (login, empty states, dashboards), not data-dense admin chrome — keep tables and forms flat for legibility.
...
**Weights:** Regular 400 (body), Medium 500 (controls, emphasis), SemiBold 600 (headings). Avoid 700+ in dense UI. Enable `font-variant-numeric: tabular-nums` for all numeric/currency columns so billing figures align. **In User Management specifically,** use `--font-mono` for `user_id`, the Entra object id (`provider_account_id`), session-token excerpts, and `AUDIT_LOG` timestamps, and enable `tabular-nums` on `failed_login_count` and any count column so the Users and Audit Log tables stay aligned.
...
- **Module badges (§3.4–3.7)** render dark `-fg` text on the light `-bg` tint — never white-on-tint. The two danger states (`DISABLED`, Locked), `DELETED`, and the auth/role/level tags are differentiated by icon and label (ban / lock / archive / shield-check / key), so meaning never depends on color alone.
