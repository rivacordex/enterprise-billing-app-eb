...
**Status:** Authoritative. Changes to *Invariants* require a documented design review.
...
All in Postgres; no file storage or cache tier in v1. Column schema is in the overview's *Data Model*; this is "what lives where." Better-Auth's managed fields map to snake_case columns via field mapping, declared once in `auth/` (Inv. #19).
...
The user-facing flow is in the overview's *Core User Flow*; this is the enforcement contract.
