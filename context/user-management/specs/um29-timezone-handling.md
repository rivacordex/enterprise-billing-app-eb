...
> **Sequencing / dependency (architecture §7 of the source plan):** This unit is written as a **delta on um28**, which introduces `lib/locale.ts`, `services/system-config/app-config-read.service.ts`, and the `formatDatetime(date, locale, fallback?)` thread-through. **As of writing, neither um28 nor um29 is in the codebase** — `lib/formatters.ts` still exports `formatDatetime(date, fallback = "Never")` with hardcoded `en-GB` + `timeZone: "UTC"`, and the `lib/locale.ts` / `app-config-read.service.ts` files do not exist. **Recommended order: ship um28 first, then um29** — um29 then only _extends_ the existing signature/service/constants module (a clean delta). If um29 ships standalone, it must additionally create `services/system-config/app-config-read.service.ts` and a constants module itself; every "extend the existing X" instruction below has a **standalone fallback** noted inline. Either order works; um28 → um29 minimizes rework.
