...
- `--action-ghost-hover` (`#EEF0F4`, near-white) is a **light-surface-only** token — it's meant to sit behind dark text. Never use it as a hover/active background on dark chrome (`--surface-nav`, `--surface-topbar`, or any element with `--text-on-brand` text): the near-white fill against near-white text reads as invisible. On dark chrome, hover/pressed states use `--color-primary-700` instead (the convention already followed by the sidebar collapse toggle) — text stays `--text-on-brand`.
