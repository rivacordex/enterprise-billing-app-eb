...
| `registration_number` | text NULL, **UNIQUE** constraint named `organization_registration_number_unique` (plain unique — Postgres already treats multiple NULLs as distinct, giving nullable-unique with no partial index needed, Module Inv. #8) — this is the exact name `cm07`/`cm08` pass to `isUniqueViolation` |
| `tax_id` | text NULL |
| `industry` | text NULL |
