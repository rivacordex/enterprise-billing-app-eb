...
Unlike every panel-embedded contact/status mutation action in this module (which stay on the same edit page and need a typed `FORBIDDEN` result for inline UI feedback), this action backs a dedicated full-page form (`/customers/manage/new`). `requirePermission`'s denial path is a `redirect()` call, and Next.js's App Router specially unwinds that `NEXT_REDIRECT` throw through a Server Action into an actual client-side navigation — the correct behavior here is a real redirect to `/no-access`, not a caught-and-converted `FORBIDDEN` result. So `requirePermission` is called with **no surrounding `try`/`catch`** — its throw (redirect or otherwise) propagates unchanged. `CreateCustomerActionResult` therefore has no `FORBIDDEN` variant (verified: `tests/actions/create-customer.action.test.ts`'s "propagates the guard's redirect and never calls the service" asserts the call rejects with `NEXT_REDIRECT`, not a caught result).

```ts
export async function createCustomerAction(rawInput: unknown): Promise<CreateCustomerActionResult> {
  const { userId: actorId } = await requirePermission(PERMISSIONS.CUSTOMERS, LEVELS.EDIT)
