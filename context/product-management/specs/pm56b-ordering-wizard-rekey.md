...
**Depends on:** pm46–pm55 (the reshape), **a fresh G-G grant**, and — because two shape questions are genuinely open — the D2 stop-and-ask below being answered first. Independent of pm56a; guardrail 31 turns fully green only once **both** land.
...
- [ ] Fresh **G-G** grant recorded (this spec header + `pm00-build-plan.md` gate table + Appendix A/W row), naming these exact files, before any edit.
- [ ] The two D2 questions answered by the user and recorded here, before code.
- [ ] `npx tsc --noEmit` — the four ordering-wizard files are gone from the error set; combined with pm56a, the repo-wide count reaches **0** (the G-E close-out's typecheck half).
- [ ] Guardrail 31 (`pricing-component-guardrails.test.ts`) is **green** — no offenders, production or test (this is the point at which it flips; assuming pm56a has landed).
- [ ] `tests/components/new-order-wizard.test.tsx` and the wizard component suites green; the New Order Wizard places a standard order with a negotiated override end-to-end (pm29's own visible result, restored).
- [ ] `eslint` / `prettier --check` clean; no schema/migration/service/repository/override-table change (`git status` — `components/products/ordering/**` + its tests only).
- [ ] Inv. #16/#38/#39 re-confirmed: override table unchanged, no `update*`/`delete*` added, `priceType`/`price_type` not conflated.
