...
- Step reads use other modules' **services** (architecture §2), but a client component may not import `services/**`/`auth/**` directly (ESLint `boundaries/dependencies`), so those reads are wrapped by Server Actions in `actions/accounts/new-order-wizard-reads.ts` — the wizard's read boundary — and the client calls those actions, never the services: `wizardSearchCustomersAction` (Step 1, `searchCustomers`, customer module), `wizardGetCustomerAccountsAction` (Step 2, `getFinancialAccountDetail`/`search-accounts`, accounts module — BAN list with state/currency/cycle/terms), `wizardSearchOfferingsAction` + `wizardGetOfferingDetailAction` (Step 3, `listOfferings` + `getOfferingDetail`, catalog — picker filter applied server-side: `ACTIVE ∧ billing_only ∧ is_sellable`, re-enforced on the detail read too). Each of the four enforces `requirePermission(PERMISSIONS.PRODUCT_ORDERS, LEVELS.EDIT)` — the same gate `createOrderAction` requires — before touching a service. (Placement rationale — why `actions/accounts/` rather than `actions/ordering/` — is in the progress tracker's pm29 note.)
...
- `wizard-step-customer.tsx` — `WizardStepCustomer`: a single name/trading-name search box (customer-search pattern), result rows; non-`ACTIVE` parties render disabled with status pill and "Not orderable — customer not ACTIVE". (`searchCustomers` has no party-role-id lookup mode to toggle onto — unlike `searchAccounts`'s `customer`/`name` modes — so one working search box, not a non-functional party-role/name toggle; see the progress tracker's pm29 "Interpretive call, flagged" note.)
...
- Form stack: `react-hook-form` + a local zodResolver schema (`wizardFormSchema`, validating `WizardFormValues` — the `useFieldArray`-friendly array shapes for characteristics/overrides), fast-fail, `Controller` for non-native inputs — all existing house patterns. `wizardFormSchema` mirrors `createOrderSchema`'s rules rather than reusing it (it shares the wire schema's `MONEY_2DP_REGEX` and `inclusiveBilledDateSchema`); the validated `WizardFormValues` are translated to `CreateOrderInput` at submit, and only then does the Server Action re-validate with `createOrderSchema` — the same local-schema-plus-translation call `price-form.tsx`/`specification-form.tsx` make.
