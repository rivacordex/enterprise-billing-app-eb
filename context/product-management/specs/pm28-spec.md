...
5. Precondition failure inside the transaction → abort, return `{ok:false, code}` — no order row persists (Q1). A post-validation write failure that must be surfaced (unexpected repo error after order insert) → order persists `FAILED` with `failure_reason` in a compensating step, matching the plan's FAILED semantics; anything earlier simply rolls back.
