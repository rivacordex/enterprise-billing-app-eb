...
1. A RevOps user can complete the full flow — search Sample Telecom, select BAN000001, order 5 × Enterprise SIP Trunk v2 starting 2026-07-21 — and one `COMPLETED` order plus one `ACTIVE` subscription with matching characteristics exist afterward, created in a single transaction (verified by integration test asserting no intermediate committed state).
