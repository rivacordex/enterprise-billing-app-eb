...
**The active work is the Rate Card Lookup update** (`product.ratecard_version`, `product.RATECARD_RAN_USAGE_LKP`, `/products/rate-card`). Authoritative decisions (the RC-series, as revised), invariants **RV1–RV4 / RV9** and open items **OR3 / OR4 / OR7′ / OR-RET** live in `_updatemodule-ratecard-lookup-plan-v2.md`; the Module Invariants it introduces are **#45–#60** in `prodmgmt-architecture.md` §6 (several withdrawn in place under v2). **This update stands up the lookup table and its upload/diff/activate/rollback/carry-forward lifecycle and its UI, and stops.** Nothing consumes the table: the rating engine's resolution of a UDR against the lookup — the `rp.py` card join, `RATECARD_LOOKUP_MISS`, the `udr_rated` stamps, drift detection — is a **following-sprint** deliverable, owned by Rating Management (`rm`-series), and you build none of it (§3.14).
...
| pm62 | **Repository** — `db/repositories/ratecard.ts`: `insertVersion`, `insertLookupRows`, `setVersionStatus`, `carryForwardRetiredRows`, the reads. No row-level write of any name. | Guardrail 37 asserts the exported surface; the surface must exist before a service can be judged against it. |
| pm63 | **Upload** — `services/product/ratecard/upload-version.ts` + its action + RV8's one set-based query. | One mutation, one audit event, one permission level (general §3.3). |
...
| pm65 | **Activate + carry-forward** — `activate-version.ts` + its action. | The subtlest piece in the update. It gets its own unit, its own tests and its own review. |
...
2. **Cite the authorizing decision before coding.** Name the RC (RC1–RC17), the RV (RV1–RV9), the Module Invariant (#45–#60 are this update's), the code-standards rule, or the Part 5 unit that mandates what you are about to build. No citation, no mandate — stop and ask.
...
- Never cache, memoise or snapshot the `ACTIVE` version — no `unstable_cache`, no `revalidate`, no React `cache()`, no module-level store, in the app or in the engine (Inv. #59). A cache re-introduces the snapshot semantics RC5 knowingly gave up and turns the version stamp on a rated record into a lie.
- Never derive `retired_at` from a clock. No `now()`, `current_date`, `new Date()` or transaction timestamp appears anywhere in the carry-forward path; it is always the new version's `snapshot_date`, always `COALESCE`d against the source row (Inv. #49).
...
- Never read a `SUPERSEDED` version on a resolution path (Inv. #48). The version list, the row preview and the diff read by id and are the only exceptions.
...
- Never reshape `ordering.order_item_price_override`, and never reason about it here (RC16, Inv. #16/#39). Out of scope means **unchanged** — this update neither reads nor touches it.
...
7. **Do not fan out per row.** RV8 is **one** set-based query over the distinct `lkp_subscriber_ref_id`s, joined in memory. No per-row query, no `Promise.all` over rows, no concurrency-limited mapper — §1.16's prohibition applies to the upload path exactly as to a list page.
8. **Do not compute the diff in SQL.** 5,500 rows against 5,500 keyed on four columns is a map comparison in memory. No set-based SQL diff, no temp table. The **carry-forward** is the opposite and runs as one `INSERT … SELECT` inside the activation transaction (code-standards §6.33).
...
10. **Do not re-validate carried-forward rows** (RV8, code-standards §1.39). Their subscriptions are legitimately terminated by then, and a validator that re-checks the incoming set blocks every future activation the first time a polygon's subscription ends — failing by *refusing*, which reads as a data problem rather than the code bug it is.
...
3. **Split every mutation.** Upload, activate and rollback are three units with three actions, three audit events and three test suites. Activate carries the carry-forward and is never merged into upload — the outgoing `ACTIVE` can change between the two (a rollback), and carrying against a stale baseline drops rows with no error anywhere (RC17).
...
2. **Stop and ask on these open items. Do not resolve one yourself, and do not implement one because a test would be easier.**
   ...
   - **OR5 — card identity.** One card system-wide, or several distinguished by `card_name`? Should activation validate that some offering actually references the name? `rateCardLookUp` is free text with no referent today. *Owner: user.* **Do not add the validation on your own initiative** — it changes what an activation can refuse.
   - **OR7 — column mapping.** `SITE ≡ polygon_id` is the least certain assumption in the design and the most damaging if wrong; `lkp_subscriber_ref_id ≡ product_inventory_id` also needs confirming. **Confirm both with RevOps before the upload contract's column set is written into `ratecard.schema.ts`.** If `lkp_subscriber_ref_id` turns out to be a customer-facing reference rather than the internal inventory ID, the referential check RV8 describes changes shape — that is a design change, not a mapping tweak. *Owner: RevOps.*
   ...
   - **OR1, OR2, OR8 — closed** 2026-09-22 into RC15, RC17 and RC4. Do not re-litigate them.
...
5. **Never guess the effectivity window.** `polygon_start_date <= event_time`, closed on the right by the next `polygon_start_date` for the same key **and** by `retired_at`. `snapshot_date` never participates in matching. Three clauses, exactly as architecture §3.3 writes them.
6. **Never guess the carry-forward.** `retired_at = COALESCE(source.retired_at, new snapshot_date)`, computed from data and never a clock, applied only to keys present in the outgoing `ACTIVE` and absent from the upload, never re-validated. Four properties; each one fails silently if missed.
...
8. **`ordering/**` and `inventory/**`** — untouched, in every unit. RV8 **reads** `inventory.product_inventory` through a repository read; it writes nothing and reshapes nothing. `ordering.order_item_price_override` is not read, modified, reshaped or reasoned about (RC16).
...
9. **Cross-module doc edits need explicit approval — and none is owed by this update.** The rating-side amendments (RC5's exception to `rm08` D4 in `ratemgmt-architecture.md`, and `ratemgmt-progress-tracker.md`'s "subscription lookup table" open question) belong to the following-sprint rating work that consumes the card, not here. Do not write into the rating module's docs from this update.
...
3. **Upload refusals create no version row.** Each of these is refused with a row-level report and **nothing written**: a duplicate `(mno, cu, polygon, polygon_start_date)`; a `Date` column that varies across rows; a missing expected column; an unknown column; an unknown `lkp_subscriber_ref_id`; status `TERMINATED` / `CANCELLED` / `ABORTED`; a `polygon_start_date` outside the subscription's window.
4. **Warnings do not block.** `SUSPENDED` or `PENDING_*` surfaces on the draft review and activation still proceeds. A `file_checksum` match warns and never refuses.
...
6. **Activation is correct in all four respects.** The `DRAFT` is promoted, the prior `ACTIVE` demoted to `SUPERSEDED`, every key present in the outgoing version and absent from the upload is carried forward with `retired_at = COALESCE(src.retired_at, new snapshot_date)`, and `carried_row_count` is written — all in one transaction under one row lock. Assert the already-retired case explicitly: that row keeps its original date. Assert `incoming keys ⊇ outgoing keys` (RV9).
...
9. **Batching and budget.** Rows insert in 1,000-row batches from the single `RATECARD_INSERT_BATCH_SIZE` constant, inside one transaction. The upload's referential check is **one** set-based query. The page query budget holds — one versions query plus its count on first render, one paged rows query plus its count on selection, two full reads for a diff — **including after a failed upload**.
10. **Audit.** Exactly one event per upload, activate and rollback, in the same transaction as the write, using the three new types; the activation payload carries the superseded version id, `carried_row_count` and the four diff counts. Page reads, row previews and diff reads write nothing. The non-`tsc`-caught filter assertion is updated (§7.4).
...
13. **Data layer.** SQL only in `db/**`; the card repository exports no row-level update or delete; every status-gated decision reads on `tx`, locked; no resolution path selects from a `SUPERSEDED` version; no cache wraps a card read and the page is `force-dynamic`.
