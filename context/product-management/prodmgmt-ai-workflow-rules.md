...
3. **G-C — Fresh, written authorization to edit `0006_product.sql` in place, granted per edit, never standing.** §6.2 closed the D11 one-round exception at pm45 and restored forward-only. PC14's second in-place edit was granted 2026-09-21 (pm46 landed it; code-standards §6.14, Appendix A row A6) and a third in-place edit (the `pricing_steps_ok` cast-safety fix) was granted 2026-09-22 (`prodmgmt-progress-tracker.md`, pm46–pm48 code-review note) — both consumed, neither a standing waiver. **Any further edit to `0006_product.sql` still needs its own fresh, written grant before the file is opened**, and **do not substitute a forward migration on your own initiative** — the choice between the two is the user's, not a detail you may settle. If the fresh-install assumption is withdrawn, the create-new-type-and-swap form (architecture §3.4) is the only correct shape and pm46's unit text changes accordingly.
...
6. **G-E — The planned red-tree window contradicted the general doc. RESOLVED (2026-09-21).** `pm00-build-plan.md` Part 4 states plainly that `db:seed` is red from pm46 until **pm48**, and that `tsc --noEmit`, ESLint and the full test suite are red from pm46 until **pm49**. General doc §1.3 says *"Never commit a unit that breaks the build"* and §3.7 says *"A step would leave the tree red — re-cut boundaries so every step is independently green."* This supplement could not grant that exception on its own; the general doc outranks it. The cause was real and was not a planning failure — PC14's column drops break code and fixtures that cannot compile against the old shape and the new one at once (architecture §3.7) — so the resolution was the user's, chosen from exactly three:
   ...
   **Chosen and recorded: the second option** — an explicit, time-boxed suspension of §1.3/§3.7, granted 2026-09-21 and widened to the **pm46–pm54 window** (`prodmgmt-code-standards.md` §6.22), with full-suite-green claimed once, at pm54. pm46–pm51 have since landed inside this window (`prodmgmt-progress-tracker.md`); the suspension remains in force through pm52–pm54 and is not renewed or re-litigated per unit.
...
2. **This update has no cross-module reach — not even a read — beyond two flagged exceptions, each authorized and delivered as its own unit.** No unit reads or writes `ordering/**`, `inventory/**`, `billing/**` or `rating/**` without the same treatment; a unit that finds itself doing so with no such authorization is out of bounds and must stop. **The exceptions were live doc-vs-doc conflicts, not permissions:** `services/ordering/order-preconditions.ts:95` resolved an override target with `price.priceType === override.priceType && price.pricingModel === "flat"`, and `workflow-management/worker/workflow-engine/runtime/rp.py`'s price-resolution window read the same dropped columns — both properties vanish when pm46 drops the columns, so `tsc`/runtime SQL break in folders this rule otherwise forbids. Architecture §3.7 said those readers ship with this update; this section and §6.5 said they must not be touched without authorization. **Resolved: both re-keys were authorized under gate G-G, granted 2026-09-22 (Khek), and delivered — `services/ordering/order-preconditions.ts` + `validation/ordering/create-order.schema.ts` (comments) by **pm50**, and `rp.py`'s `price_windows` CTE/resolution SELECT by **pm51** — each naming its exact files, each a re-key with no compatibility shim (code-standards §1.34/§6.21, Appendix A rows A9; Appendix W row W4 for pm50).** No other cross-module file is touched; a unit finding a new crossing still stops and gets its own G-G authorization first.
...
| W3  | §1.3/§3.7 of the **general** doc — every unit lands green — as applied to pm46–pm49.                                            | **RESOLVED (2026-09-21).** The time-boxed suspension option was chosen and recorded (§0.6), window widened to pm46–pm54, full-suite-green claimed once, at pm54.                                                            | gate G-E ✓  |
| W4  | ~~§3.2 — no cross-module reach at all, as applied to `services/ordering/order-preconditions.ts:95`.~~ **CLEARED (pm50, 2026-09-22).** | G-G authorization granted 2026-09-22 (Khek), naming `services/ordering/order-preconditions.ts`, `validation/ordering/create-order.schema.ts` (comments only) and the four ordering integration test fixtures, stating the crossing is a re-key with no behaviour change. Delivered by pm50 (code-standards Appendix A9). | pm50 ✓      |
