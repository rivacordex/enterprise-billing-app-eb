...
| `services/ordering/order-preconditions.ts` | `app_runtime` | `price.priceType === override.priceType && price.pricingModel === 'flat'` to validate an override target | Override target resolution must re-key to `component_type` (`usage_rate` / `flat_fee`), including the `priceType`/`pricingModel` reads and the flat `pricing_model` documentation. Application code, but deferred — not built in this update; requires separate authorization before the Ordering reader changes. |
| `validation/ordering/create-order.schema.ts` | `app_runtime` | documents the `flat` + `price_type` contract in comments | Contract text updated; the DB CHECK it mirrors is on the `ordering` table and is unchanged. |
