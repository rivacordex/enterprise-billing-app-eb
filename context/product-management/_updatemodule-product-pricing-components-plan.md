...
- **O10** — Not-yet-billable UI warning for the new component types (the pm38 pattern for tiered recurring / tiered usage) until the bill-run phase supports them. Owner: Manage Products UI phase. **Delivered pm55 (2026-09-23):** the four surviving warning copies (`capacity_commitment`, `capacity_motivation`, the absent-rate-card `usage_rate` case, and the `Mbps` basis placeholder — see H2 in `specs/pm00-build-plan.md`) render inline under the component row in `components/products/manage/editable-prices.tsx`, warning-tinted, non-blocking.
