...
2. **Money arithmetic lives in exactly one file: `services/accounts/money.ts`.** Platform §2.15/§6.16 require a design decision before money arithmetic — this is it: amounts stay `string` end-to-end; `money.ts` converts to integer **sen** (minor units) for add/subtract/compare, returns `string`, and throws a plain `Error` with message `'MONEY_PRECISION'` on >2 decimal places — a programmer-contract guard, not a user-facing error. No `parseFloat`, no `Number()` on an amount anywhere else, including tests.
...
4. **Module services return `Result`, never throw.** No `services/accounts/**` file constructs or throws an `AppError` — the module's failure vocabulary is the `{ ok: false, code }` catalog in **§9**, which is distinct from the platform's thrown `APP_ERROR_CODES` (`lib/errors.ts`). The two never mix: a Result code is never mapped to HTTP directly, and `AppError` never crosses a service boundary. Codes are stable machine strings; adding one is a documented change to §9, not an ad-hoc string. _(Exception: `money.ts` throws a plain `Error('MONEY_PRECISION')` as a programmer-contract guard when an amount with more than two decimal places reaches arithmetic — not an `AppError`, not a Result code, and not reachable from validated action inputs; Zod schemas at action boundaries prevent it from firing in production.)_
...
| `INV_NOT_REVERSIBLE` | `reverse-document`, `reverse-line`, `get-reversal-preview` | Target is an `INV` document — billing owns its finalization latch (`customer_bill.ref_inv_document_id`), so INV reversal is out of scope (bm11); correct via a credit note |
