...
- **Note on codebase verification:** planning-folder-only session. Confirm the Administration section's page-registration pattern (mirror the existing `administration/**` pages) and the `system_config` write path for the wizard-default rows (ac03 seeded them; ac15 edits via the config repository).
