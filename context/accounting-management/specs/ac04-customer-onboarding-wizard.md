...
- **Implementation verified:** (a) `cm10`'s `transitionCustomerStatus` is not called directly — the **service** (`services/accounts/onboard-customer-accounts.ts`) owns the single `db.transaction`; the action (`actions/accounts/onboard-customer-accounts.ts`) cannot import from `db` (ESLint `boundaries/dependencies` rule: actions → db is prohibited). The service performs all steps: pre-checks (PARTY_ROLE_NOT_FOUND, INVALID_TRANSITION) before opening the transaction, then inside it: cycle check, `compareAndUpdateStatus(→ VALIDATED)`, FA insert, BAN insert, `pgledger_create_account` ×3, three `ledger_binding` rows, and the `ACCOUNTS_ONBOARDED` audit event. The action delegates fully via `onboardCustomerAccounts(input, actorId)`. (b) The VALIDATED-transition wizard is injected into `cm10`'s `CustomerRoleForm`/`StatusTransitionControl` via a deferred Promise in `handleStatusTransition`, with `OnboardAccountsWizard` mounted as a sibling dialog; non-VALIDATED transitions are unaffected. (c) `ref_party_role_id` references `customer.party_role.party_role_id` (text, `PTRL…` prefix).
...
Q2 is explicit: "Status change + FA/BAN creation + ledger accounts + bindings commit in one transaction." So the VALIDATED transition and onboarding are **one atomic unit**, not two sequential saves. Design: the onboarding **service** (`services/accounts/onboard-customer-accounts.ts`) owns the single `db.transaction(tx => …)`. The action (`actions/accounts/onboard-customer-accounts.ts`) cannot import from `db` (ESLint `boundaries/dependencies` rule enforces the inward-only import graph: actions → db is prohibited; actions may only import services, auth, validation, types, lib). The service performs every step: pre-checks before opening the transaction (PARTY_ROLE_NOT_FOUND, INVALID_TRANSITION) for a fast-fail path, then inside the transaction: (0) cycle-state check (CYCLE_RETIRED), (1) `compareAndUpdateStatus` to set `party_role.status = VALIDATED`, (2a–2d) FA/BAN/ledger accounts/bindings, (3) `ACCOUNTS_ONBOARDED` audit event. The action delegates by calling `onboardCustomerAccounts(input, actorId)`. Any failure inside the transaction — a pgledger error, a binding conflict, a stale lock — rolls back **everything**: no VALIDATED status without accounts, no accounts without VALIDATED, no orphan FA/BAN/ledger/binding (V7).
...
- **`onboardCustomerAccounts(tx, input, actorId)` returns `Result`** (code-standards §2.4) for expected business outcomes — `CYCLE_RETIRED` (picked cycle no longer active). `CONFLICT` (stale party-role lock), `INVALID_TRANSITION` (not on the VALIDATED edge), and `PARTY_ROLE_NOT_FOUND` are surfaced by the **action** (not the service), since the action owns the pre-checks and the status-change step. Uncaught repository, ledger, and audit infrastructure failures may throw; these propagate out of the caller's `db.transaction` and roll back all writes. The wizard maps each result code to a field/banner.
