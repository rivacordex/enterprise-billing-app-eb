-- ac02: the 10 module tables (financial_account, billing_account,
-- bill_cycle, reason_code, document, document_line, ledger_binding,
-- gl_account, gl_mapping, accounting_period) generated by `drizzle-kit
-- generate` from db/schema/billing/**, plus the 3 raw-SQL composition views
-- (account_view, gl_resolution_view, gl_journal_view — db/schema/billing/
-- views.sql, hand-appended below per ac02-spec §3.2/§3.3, since Drizzle
-- can't declare a view joining the pgledger fork).
--
-- `IF NOT EXISTS` (drizzle-kit's own generated form is a bare `CREATE
-- SCHEMA "billing"`): the schema already exists from migration 0011
-- (billing_pgledger) — `billing` was only added to drizzle.config.ts's
-- schemaFilter in this unit (ac01 deliberately left it out, see
-- acctmgmt-progress-tracker.md Session Notes), so this is drizzle-kit
-- seeing the schema as "new" against its own prior snapshot even though the
...
CREATE TABLE "billing"."document" (
	...
	CONSTRAINT "document_payment_mode_check" CHECK (payment_mode IS NULL OR payment_mode IN ('bank_transfer','cash','cheque'))
);
--> statement-breakpoint
...
ALTER TABLE "billing"."document" ADD CONSTRAINT "document_reversal_of_document_document_id_fk" FOREIGN KEY ("reversal_of") REFERENCES "billing"."document"("document_id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "billing"."document_line" ADD CONSTRAINT "document_line_reversed_by_line_id_document_line_document_line_id_fk" FOREIGN KEY ("reversed_by_line_id") REFERENCES "billing"."document_line"("document_line_id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
-- ---------------------------------------------------------------------------
-- The 3 composition views (db/schema/billing/views.sql) -- hand-appended,
-- never re-typed. See that file for the design rationale of each view.
-- ---------------------------------------------------------------------------
