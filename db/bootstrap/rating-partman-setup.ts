...
// One-shot provisioning CLI (`npm run db:setup-partman-rating`). Never
// imported by application code. Stands up pg_partman for rating.udr_rated
// and rating.process_log by executing db/bootstrap/rating-partman-setup.sql
// (rm01-spec §Implementation §6) — mirrors db/bootstrap/billing-partman-setup.ts
// exactly.
//
...
// created the partitioned parents (0034_rating.sql).
const PARTMAN_SQL_PATH = join(import.meta.dirname, "rating-partman-setup.sql");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.info("Rating pg_partman setup applied successfully.", {
...
void main().catch((err: unknown) => {
  const pgErr =
    typeof err === "object" && err !== null
      ...
      : {};
  logger.error("Rating pg_partman setup failed.", {
