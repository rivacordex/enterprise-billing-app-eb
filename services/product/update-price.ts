...
// pm38-spec I4, amended pm41 review #1. Corrects a price on a DRAFT version —
// never branches (D4): a request to change a specific existing price row on a
// released version is not a request to create a new one, so the repository
// refuses a non-DRAFT parent (`OFFERING_NOT_DRAFT`) and the UI tells the user to
// edit the open draft instead. `now` defaults to the real clock but is
// injectable for deterministic tests, mirroring insert-price.ts.
//
