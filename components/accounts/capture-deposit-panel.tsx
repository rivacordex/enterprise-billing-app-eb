...
import { captureDepositAction } from "@/actions/accounts/capture-deposit";
...
export interface CaptureDepositPanelProps {
...
export function CaptureDepositPanel({
  financialAccountId,
}: CaptureDepositPanelProps): React.JSX.Element {
    ...
    <section className="space-y-3 rounded-md border border-[color:var(--border-default)] bg-[color:var(--surface-card)] p-4">
      ...
      {disabled && (
        <p className="text-body-sm text-muted-foreground">
          Select a Financial Account in the context strip to capture a deposit.
        </p>
      )}
      ...
      <fieldset disabled={disabled || submitting} className="space-y-3">
        ...
        >
          {submitting ? "Capturing…" : "Capture Deposit"}
        </Button>
      </fieldset>
    </section>
  );
...
function describeCaptureDepositError(code: string): string {
  switch (code) {
